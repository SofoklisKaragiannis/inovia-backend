...
@CrossOrigin
@RestController
@RequestMapping(V1.URI_UPDATE_PRODUCT_ABSOLUTE)
public class UpdateProductController {
    ...
    public DeferredResult<ResponseEntity<ProductModel>> updateProduct(@RequestBody Product product,
        ...
        if (product != null && product.getId() != null && !productService.findProductById(product.getId()).equals(Optional.empty())) {
            productModel = StaticContent.productEntityToModel(productService.modifyProduct(product));
        } else {
